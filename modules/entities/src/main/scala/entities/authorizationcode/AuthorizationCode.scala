...
case class AuthorizationCode(id: AuthorizationCodeValue,
  ...
  def authenticate(redirectUri: Option[String], clientId: ClientId): ValidationResult[AuthorizationCode] =
    (validateRedirectUri(redirectUri), validateClientId(clientId)) mapN {
      case (_, _) => this
    }
